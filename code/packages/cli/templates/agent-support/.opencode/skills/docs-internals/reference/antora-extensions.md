...
| --------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| module switcher       | `nav_modules:` in `docs/docs/antora.yml`                          | stamps each nav tree with its module/title/description/icon, so the UI can show one module's nav at a time with a switcher — see `reference/page-patterns.md`'s "Mono-module vs. multi-module" section. A no-op until `nav_modules:` is authored                                                       |
| site footer           | `footer:` in `docs/docs/antora.yml`                               | resolves a structured `footer: groups: […]` list of link groups and attaches it to the component version, for the UI's footer partial. A no-op until `footer:` is authored                                                                                                                             |
...
| `llms.txt` generation | `llms:` in `docs/docs/antora.yml` (optional `summary`/`exclude`)  | generates `llms.txt` and `llms-full.txt` at the site root — a Markdown index and full dump, meant for LLM ingestion (see `https://llmstxt.org`), so an AI agent (or another tool) can read the site without scraping rendered HTML                                                                     |
...
All three authored keys (`nav_modules`, `footer`, `llms`) live in `docs/docs/antora.yml`,
