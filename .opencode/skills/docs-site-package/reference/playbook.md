...
| `latest_version_segment` | unset | a fixed segment such as `latest` aliasing the current version — DO NOT set this on a `pdocs new`-scaffolded site: its `replace` strategy turns one real version segment into a redirect stub (GH #137); those sites instead opt `@inditextech/pdocs-antora-extensions`'s `duplicateLatestVersion` into republishing the latest version's content a second time under `/latest/…`, keeping both segments real (see `reference/versioning-modes.md`) |
