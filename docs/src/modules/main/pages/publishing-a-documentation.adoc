...
A `cname` string under `package.json`'s own `"docouture": { "publish": { "gh-pages":
{ "cname": "docs.example.com" } } }` — `docouture publish` writes it as a `CNAME` file
alongside the built site. Leave the block empty, as scaffolded, for a plain
`github.io` URL.
