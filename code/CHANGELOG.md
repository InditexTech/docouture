...
- [#142](https://github.com/InditexTech/test-antoradocs/issues/142) Renamed the product from `pdocs` to `docouture`: npm packages are now scoped `@inditextech/docouture*` (e.g. `@inditextech/docouture-cli` was `@inditextech/pdocs-cli`), the CLI binary is now `docouture` (was `pdocs`), the Antora extension logger-name prefix is now `docouture-*` (was `pdocs-*`), and CLI-scaffolded output (workflow templates, agent skill docs, the site-title placeholder token) uses `docouture` branding for newly created sites. Entries below this one predate the rename and refer to the product by its former name.
...
- [#139](https://github.com/InditexTech/test-antoradocs/issues/139) Polish the pdocs CLI against Node.js CLI best practices: per-subcommand `--help` (`pdocs new --help`, etc.), a banner printed on every invocation (not just `--version`/errors, suppressed under `--json`), global `--json` (currently `doctor --json`), `--verbose`/`DEBUG=pdocs` diagnostic logging, and `--no-color` — the last backed by a new `lib/theme.ts` that re-checks colour support on every call rather than at import time (replacing `picocolors` and `doctor`'s own hand-rolled ANSI, neither of which could react to a flag parsed after they loaded); `ora` spinners for `pdocs dev`'s startup and `pdocs publish`'s driver call; cross-platform `npm.cmd`/`antora.cmd` spawn resolution and SIGINT/SIGTERM forwarding/child-process cleanup for `pdocs build`/`pdocs dev`; a shared `lib/config-resolver.ts` precedence merge used by `publish`/`upgrade`; a new `pdocs completion bash|zsh` command; and a `cli-best-practices` agent skill documenting these conventions for future changes. Deliberately does not adopt a CLI framework (commander/yargs) — see the skill for why.
