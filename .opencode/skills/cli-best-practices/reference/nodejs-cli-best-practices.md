# Node.js CLI best practices — checklist applied to docouture
...
items actually relevant to a single-binary, no-daemon CLI like `docouture`, with
...
| Helpful help on bad/no invocation | Done | bare `docouture` prints help and exits 0 (not an error — matches git/npm) |
...
| Shell completion | Done | `docouture completion bash\|zsh` (`src/commands/completion.ts`) |
...
| Debug mode | Done | `--verbose` / `DEBUG=docouture` (`lib/debug-log.ts`) |
| Exit codes | Done | every command returns 0/1; `process.exitCode`, never an abrupt `process.exit()` |
| Actionable errors | Done | most failure messages name the next command to run (`docouture new` first, `npm install --save-dev <driver>`, etc.) |
...
- **Docker distribution.** `docouture` is only ever installed as a devDependency
