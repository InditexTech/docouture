...
// pdocs's own Antora pipeline extensions (kroki-prewarm.js, search-index.js,
// llms-txt.js, footer.js, nav-modules.js, not-found-page.js,
// version-report.js — anything calling `context.getLogger('pdocs-...')`)
// logs its own healthy-run observability at `info` and is silently dropped
// before it ever reaches stdout. `pdocs build`/`pdocs dev` pass this to
...
// `pdocs-*` named logger regardless of level (every current and future
// pdocs extension follows that naming convention — see the file list
// above) — pdocs's own healthy-run observability, which is otherwise
// indistinguishable from Antora's own internal `info` chatter that
// `--log-level=info` also just turned on.
const OBSERVABLE_LINE = /"level":"(?:warn|error)"|"name":"pdocs-/
