...
Ordered by how often it comes up here, not by upstream nav order. Antora-specific
behaviour is called out inline; everything else is plain AsciiDoc as Asciidoctor
implements it.
...
- `.role` — a CSS class. Repeatable. This is the hook for design-system styling.
...
- The first positional attribute is the **block style**, which can make one delimiter
  behave as another (`[quote]` on `====`, `[source]` on `----`). This is *block
  masquerading*.
...
xref:page.adoc[Text]                          Antora resource ID — see SKILL.md
xref:page.adoc#section-id[Text]
xref:#local-section[Text]                     same page
<<local-section,Text>>                        same page, alternate form
```

- Empty xref text (`xref:page.adoc[]`) renders the target page's title. Prefer it —
  it stays correct when the title changes.
...
- `source-highlighter=highlight.js` is Antora's default and is in force. The language
  token must be one highlight.js knows.
...
Five types only: `NOTE`, `TIP`, `IMPORTANT`, `WARNING`, `CAUTION`. A custom type requires
either a role plus CSS or a block extension.
...
Tagged regions are marked in the *included* file with comments in that file's own comment
syntax:
...
page header. A playbook attribute ending in `@` becomes *soft-set* and can be overridden
by a page; without `@` it cannot.

Counters:

```adoc
{counter:seq}                        1, then 2, then 3 on each reference
{counter2:seq}                       increments without rendering
```

`attributes/document-attributes/`, `attributes/assignment-precedence/`,
`attributes/counters/`, `attributes/document-attributes-ref/`
...
toc::[]                              macro TOC; needs :toc: macro in the header
```
...
| Normalization (encoding, line endings) | `normalization/` |
