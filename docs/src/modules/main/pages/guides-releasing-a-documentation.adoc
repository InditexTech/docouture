...
commit built on `main`'s current tip, `git tag v<value>` is created there, and the tag
is pushed — `main` itself is never advanced.

== Standalone mode

Every release retargets the same `stable` tag — force-moved, not appended to. Nothing
...
. Merge it — the workflow tags `v1.0.0`, then bumps the file forward to the next patch
