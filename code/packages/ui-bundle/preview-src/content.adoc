...
// heading scale — `toclevels` now defaults to 4 (h2-h5, toc.hbs), so this
// explicit override is redundant but kept to document the mechanism.
...
// Set `:page-toclevels: 3` (or lower) on a page to HIDE deeper levels
// instead of showing them, since h1-h5 now show by default.
:page-toclevels: 4
...
While React helpers are included, other frameworks work too, with a bit more
wiring.
====

.What about `AMIGA_PROFILE` — does casing survive?
[%collapsible]
====
Yes: this row's own title keeps its identifier casing and its bold word,
neither uppercased nor split apart by layout — see accordion.css's own
header for why that used to be a real bug.
====

.A third item, so Home/End have somewhere to jump
[%collapsible]
====
Arrow-Down/Up moves focus between headers; Home/End jump to the first/last.
Opening this one closes whichever of the other two was open — that
exclusivity is the browser's own `<details name>` behaviour, not this
page's script.
====
--
...
GH-45 — see `asciidoc-extensions/lib/tabs.js`. Any content, not just code: this
one holds prose. Every `[tabs]` block is independent — picking a tab here
does not affect the pair further down this page, or the one nested in the
step below.
...
[tab,label="pnpm"]
****
Prose works here, not just code — [tabs] holds arbitrary content.
...
A second, independent `[tabs]` block — picking a tab here does not change the
one above it:
...
A `[tabs]` nested inside a `[steps]` step — the corpus's actual shape in
`main/quickstart.adoc`:
...
The ATD Card — see `asciidoc-extensions/lib/card-grid.js`. Default shape: no
image, three per row from `m`, capped at the reading measure. The whole card is
the link — there is no button, and clicking anywhere on it navigates.

[cards]
====
[card]
.https://example.org[WeaveAction]
The abstract class that defines the blueprint of an action in Weave.js.

[card,subheader="Tooling",labels="canvas, feedback"]
.link:content.html[WeaveCommentToolAction]
Enables users to directly comment and give feedback on the stage. This one
carries a subheader and label chips.

[card,icon="actions/code-block-outlined"]
.link:content.html[WeaveEraserToolAction]
Allows users to erase elements of the canvas by clicking on them. This one
carries a header icon.
====

Four columns, images in the square aspect, and the row let out of the reading
measure to the full content column — `[cards,type=image-square,columns="2 s:4",width=container]`.

[cards,type=image-square,columns="2 s:4",width=container]
====
[card,subheader="Aspect"]
.link:content.html[First]
--
image::photo-medium.png[A canvas with a text node selected]

Every card in a block shares one aspect ratio — it is a block attribute, not a per-card one.
--

[card,subheader="Crop"]
.link:content.html[Second]
--
image::photo-medium.png[A canvas with a text node selected]

The image is cropped to fill, never letterboxed.
--

[card,subheader="Measure"]
.link:content.html[Third]
--
image::photo-medium.png[A canvas with a text node selected]

`width=container` lets the row out of the 820px reading measure.
--

[card,subheader="Hover"]
.link:content.html[Fourth]
--
image::photo-medium.png[A canvas with a text node selected]

Hovering rules the card's edges, per the design; the title never changes colour.
--
====
