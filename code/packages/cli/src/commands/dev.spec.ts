...
describe('runDev', () => {
  it('fails when no antora-playbook.local.yml is found under --dir/docs', async () => {
    const code = await runDev(['--dir', base])
    expect(code).toBe(1)
    expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('no antora-playbook.local.yml found'))
  })

  it('rejects an invalid --port', async () => {
    await mkdir(join(base, 'docs'), { recursive: true })
    await writeFile(join(base, 'docs', 'antora-playbook.local.yml'), 'site:\n  title: X\n', 'utf8')
