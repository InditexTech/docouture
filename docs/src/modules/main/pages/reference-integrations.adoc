...
site with no changelog page is left untouched. `## [Unreleased]` is deliberately
excluded, the same way Keep a Changelog itself doesn't consider it a version.

The file is read once per build, off disk, applied identically to every component
version being built — not resolved per-ref through git, so a future build that
aggregates two real docs versions side by side will show the same (current) changelog
content on both until that gets revisited. Override the default `code/CHANGELOG.md`
