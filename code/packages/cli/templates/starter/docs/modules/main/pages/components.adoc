...
`@inditextech/pdocs-asciidoc-extensions`' `lib/kroki-config.js` for the full list) render
...
This feature is **disabled by default** in this starter (see `antora-playbook.yml`'s
commented-out `kroki-enabled`/`kroki-diagram-types` attributes) — until it's turned on
and a Kroki service is actually reachable at build time, a block like the one above
renders exactly as plain AsciiDoc already would: the raw diagram source, as literal
text, same as this page shows it above. Enabling it needs the playbook attributes
uncommented and Docker available — the build starts the Kroki service itself the first
time it's needed, no manual step required (it isn't stopped automatically either — run
`pdocs teardown kroki` once you're done with it). Run `pdocs eject kroki` if you ever
need to customize the container definition.
