...
`@inditextech/docouture-ui-bundle`, `@inditextech/docouture-antora-extensions`,
`@inditextech/docouture-asciidoc-extensions` — and none of those resolve outside this
monorepo via a `workspace:*` range, so nothing about it actually worked once copied out.
The one surviving `starter` is
**`code/packages/cli/templates/starter`** — bundled into the `@inditextech/docouture-cli` npm
package at CLI build time (`scripts/copy-templates.mjs`) and copied out by `docouture new`
into a fresh directory, with `__DOCOUTURE_NAME__`/`__DOCOUTURE_TITLE__`/`__DOCOUTURE_CLI_VERSION__`
...
the exact same version as `@inditextech/docouture-cli` (the packages release in lockstep),
...
skill `docouture new` also scaffolds documents, and what makes the UI bundle's own search
...
about the CLI's bundled template and what `docouture new` does with it.

## What is in it

```
code/packages/cli/templates/starter/
  package.json                          __DOCOUTURE_NAME__, private, deps: antora + docouture-ui-bundle + the two docouture extension packages
  antora-playbook.yml                   heavily commented — docouture UI bundle via node_modules
  antora-playbook.pr-verify.yml         HEAD-only companion, used by docouture-pr-verify.yml
  docs/
    antora.yml                          name/title: __DOCOUTURE_NAME__/__DOCOUTURE_TITLE__, version: ~
    modules/ROOT/
      nav.adoc
      pages/index.adoc                  "what is here"
      pages/getting-started.adoc        add a page, build, set a version
  .github/workflows/
    docouture-publish.yml                   build (and, once it exists, deploy); called by docouture-release.yml, or manual
    docouture-pr-verify.yml                 HEAD-only build on every pull_request
    docouture-release.yml                   cuts a release for either mode — detects which
    docouture-release-preview.yml           comments the target version/tag on a docs/release-labelled PR, before merge
  .gitignore
```
...
| docouture UI bundle as an npm devDependency, `ui.bundle.url` reading it from `node_modules` | works the moment `npm install` has run, no build-time fetch from a URL, and stays pinned to the exact bundle version this template was generated with |
| two content pages only | `index.adoc` proves `site.start_page` resolves; `getting-started.adoc` proves `nav.adoc`, `xref:` and `docouture version` work. A third page proves nothing new |
| `devDependencies` include `@inditextech/docouture-cli` | `docouture version` (used in `getting-started.adoc` and `docouture-release.yml`) needs to resolve as a local binary in the scaffolded site, not just whatever's on the scaffolding machine's `PATH` |
| one `docouture-release.yml`, mode detected at run time | avoids shipping two near-identical files a site owner has to pick between and keep one of; the trade-off is a workflow that reads `docs/antora.yml` before it knows what else to do |
...
Fumadocs migration target) that also happens to trial docouture' own extensions ahead of the
...
- **A change that demonstrates a docouture-specific feature not yet in the template goes in
...
- **Keep it working standalone.** It must build with nothing beyond what a fresh
  `docouture new` output has — no `workspace:*`, no sibling packages, no assumption this repo
  exists.
...
- **Placeholders are `__DOCOUTURE_NAME__` / `__DOCOUTURE_TITLE__` / `__DOCOUTURE_CLI_VERSION__`** —
...
- **New files here appear in every future `docouture new` automatically** — `copyTemplate`
