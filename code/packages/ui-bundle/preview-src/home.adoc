...
first of them (GH-20, the `[cards]` block); until the rest land (GH-21, GH-23)
any AsciiDoc renders here, which is what this section is for: it checks that
the block's heading row, its dashed rule and the gaps around its body are
