...
is plain https://asciidoc.org[AsciiDoc] — nothing docouture-specific, all of it documented
in full in the `docouture-writing-docs-pages` skill's `reference/language-basics.md`. The
second half is this site's own custom blocks, registered by
`@inditextech/docouture-asciidoc-extensions` and documented in that same skill's
`reference/docouture-blocks.md`. This page lives under
xref:main:reference-overview.adoc[Reference] — see xref:main:features.adoc[] for
the build-time capabilities (diagrams, syntax highlighting) mentioned below, in full.
...
link]. Referring to another page on this site: xref:main:adding-a-page.adoc[] — empty
xref text renders that page's own title, so it stays correct if the title changes. A
fragment: xref:main:adding-a-page.adoc#group-without-a-page-of-its-own[a specific
