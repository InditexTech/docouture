...
`antora-playbook.local.yml` (but path-filtered: only fires when the PR touches `docs/**`
...
also path-filtered to `docs/**` + its own workflow file)
...
Both `docouture-pr-verify.yml` and `docouture-kroki-cache-warm.yml` are path-filtered to
`docs/**` + their own workflow file — neither has anything to usefully check or warm a
cache for from a change elsewhere in the repo. If `docouture-pr-verify.yml` is configured
as a required status check in branch protection, confirm that setup tolerates a skipped
run (GitHub's "Do not require status checks on creation," or rulesets' own equivalent):
a PR that never touches `docs/` now means it never reports at all, rather than reporting
green.

== A release under git-flow, end to end

. Feature branch → `develop`: `docouture-pr-verify.yml` fires on the PR if it touches
  `docs/**` (it's branch-agnostic, but path-filtered — see above). On merge,
  `docouture-publish-prerelease.yml` republishes the prerelease docs from `develop`.
. `develop` → `main`, labelled `docs/release`: `docouture-pr-verify.yml` fires under the
  same `docs/**` path filter, and `docouture-release-preview.yml` now fires too (targets
  `main`), posting a preview comment. On merge, `docouture-release.yml` cuts the release
  (checks out `main`, tags it, pushes the tag) and calls `docouture-publish.yml` to build
  and publish.
