...
GitHub settings. None of them are things `pdocs new` can do on your behalf —
...
is still not the "publish this to the world" flow `pdocs-publish.yml`
assumes). If this repository is private and is meant to stay that way,
publishing to GitHub Pages is not the right target — use `pdocs build` and
...
created the first time `pdocs publish gh-pages` (or the scaffolded
`.github/workflows/pdocs-publish.yml`, which calls it) actually runs. Publish
...
`.github/workflows/pdocs-release.yml` (scaffolded by `pdocs new`) can cut a
